...
function init() {
    ...
    // Ensure a canvas exists; if not, create one so deployment environments that inline scripts earlier still work
    ...
    loader.load('/photos/space-bg.jpg', (tex) => {
        addBgSphere(tex);
    }, undefined, () => {
        // primary missing — try a backup that exists in the repo
        loader.load('/photos/IMG_0929.JPG', (tex2) => {
            addBgSphere(tex2);
        }, undefined, () => {
            // final fallback: keep the star points and a subtle dark background
            console.warn('No background texture found at /photos/space-bg.jpg or /photos/IMG_0929.JPG; using color background and procedural stars.');
...
// Run after DOM is ready so canvas and other elements are available
